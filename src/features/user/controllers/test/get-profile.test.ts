...
describe('Get', () => {
  ...
  describe('all', () => {
    it('should send success json response if users in cache', async () => {
      ...
      jest.spyOn(UserCache.prototype, 'getTotalUsersCache').mockResolvedValue(1);
    ...
    it('should send success json response if users in database', async () => {
      ...
      jest.spyOn(UserCache.prototype, 'getTotalUsersCache').mockResolvedValue(0);
      ...
      jest.spyOn(userService, 'usersCount').mockResolvedValue(1);
  ...
  describe('profileAndPosts', () => {
    ...
    it('should send success json response if user in database', async () => {
      ...
      jest.spyOn(userService, 'getUserByUsername').mockResolvedValue(existingUser);
  ...
  describe('randomUserSuggestions', () => {
    it('should send success json response if user in cache', async () => {
      ...
      expect(UserCache.prototype.getRandomUsersFromCache).toHaveBeenCalledWith(`${req.currentUser?.userId}`);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
    ...
    it('should send success json response if user in database', async () => {
      ...
      await Get.prototype.randomUserSuggestions(req, res);
      expect(userService.getRandomUsers).toHaveBeenCalledWith(req.currentUser!.userId, req.currentUser!.username);
      expect(res.status).toHaveBeenCalledWith(200);
      expect(res.json).toHaveBeenCalledWith({
        message: 'User suggestions',
        users: [existingUser]
