...
export class ReactionCache extends BaseCache {
    ...
    public async savePostReactionToCache(key: string, value: string, postReactions: IReactions, type: string, previousReaction: string): Promise<void> {
        try {
            ...
            if (previousReaction) {
                this.removePostReactionFromCache(key, reaction.username, previousReaction, postReactions);
            }
            if (type) {
    ...
    public async removePostReactionFromCache(key: string, username: string, previousReaction: string, postReactions: IReactions): Promise<void> {
        try {
            if (!this.client.isOpen) {
                await this.client.connect();
            }
    ...
    public async getReactionsFromCache(key: string): Promise<[IReactionDocument[], number]> {
        try {
            ...
            return response.length ? [list, response] : [[], 0];
        } catch (error) {
            throw new ServerError('Server error. Try again.');
        }
    }
    ...
    public async getSingleReactionFromCache(key: string, reactionId: string): Promise<[IReactionDocument[], number]> {
        try {
            ...
            const reply = await this.client.LRANGE(`reactions:${key}`, 0, -1);
            const list: IReactionDocument[] = [];
